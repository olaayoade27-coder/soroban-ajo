...
const logger = createModuleLogger('EmailQueue')
...
emailQueue.process(async (job) => {
  ...
  switch (type) {
    ...
    case 'payout':
      return emailService.sendPayoutNotification(data.to, data.groupName, data.amount, data.txHash)
...
export const queueEmail = {
  welcome: (to: string, name: string) => emailQueue.add({ type: 'welcome', data: { to, name } }),
  ...
  custom: (options: EmailOptions) => emailQueue.add({ type: 'custom', data: options }),
}
